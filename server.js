...
app.use(cors(
    {
        origin: ["https://pnv-vespa.vercel.app"],
...
app.post('/loginClient', (req, res) => {
    ...
    con.query(sql, [req.body.email], (err, result) => {
        if(err) return res.json({Status: "Error", Error: "Error in runnig query(Server is dead, continue after fifteen minutes)"});
        if(result.length > 0) {
            bcrypt.compare(req.body.password.toString(), result[0].password, (err, response)=> {
                ...
                if(response) {
                    ...
                    return res.json({Status: "Success", Success: "I am verifying your login account, please wait a few minutes" , id: result[0].id})
                } else {
